-- 监管账号查询
set @start='2024-09-25 00:00:00';
set @end='2024-09-27 23:59:59';
SET @start_time = UNIX_TIMESTAMP(@start)*1000;
SET @end_time = UNIX_TIMESTAMP(@end)*1000;
select source,task_id,trace_id,biz_type,scene_type,from_unixtime(round(time_stamp/1000),'%Y-%m-%d %H:%i:%s') as robot_time,role,content,risk_result,JSON_UNQUOTE(JSON_EXTRACT(report_json, '$.data.review_result')) AS review_result  from (
    select JSON_EXTRACT(JSON_UNQUOTE(JSON_EXTRACT(origin_data, '$.payload')),'$.source')  AS source,id,task_id,trace_id,biz_type,scene_type,time_stamp,role,content,risk_result
...
)a left join (select * from tbl_risk_report where  create_time between @start and @end) b on a.id=b.message_id
...
-- 万知账号查询
set @start='2024-09-25 00:00:00';
set @end='2024-09-29 23:59:59';
SET @start_time = UNIX_TIMESTAMP(@start)*1000;
SET @end_time = UNIX_TIMESTAMP(@end)*1000;
select distinct account_id from (
select source,task_id,account_id,trace_id,biz_type,scene_type,from_unixtime(round(time_stamp/1000),'%Y-%m-%d %H:%i:%s') as robot_time,role,content,risk_result,JSON_UNQUOTE(JSON_EXTRACT(report_json, '$.data.review_result')) AS review_result
from (
    select JSON_EXTRACT(JSON_UNQUOTE(JSON_EXTRACT(origin_data, '$.payload')),'$.source')  AS source,account_id,id,task_id,trace_id,biz_type,scene_type,time_stamp,role,content,risk_result
